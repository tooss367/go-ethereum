...
func NewHeaderChain(chainDb ethdb.Database, config *params.ChainConfig, engine consensus.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	...
	if gHash := rawdb.ReadCanonicalHash(chainDb, 0); gHash != (common.Hash{}) {
		genesis = rawdb.ReadHeader(chainDb, gHash, 0)
	}
	if genesis == nil {
	...
	hc := &HeaderChain{
		...
		chainTracker:  NewHeaderDB(genesis),
...
func (hc *HeaderChain) GetBlockNumber(hash common.Hash) *uint64 {
	...
	if h := hc.chainTracker.GetHeaderByHash(hash); h != nil {
		n := h.Number.Uint64()
		return &n
	}
...
func (hc *HeaderChain) writeHeaders(headers []*types.Header) (result *headerWriteResult, err error) {
	...
	return hc.chainTracker.WriteHeaders(headers)
	/*
		if len(headers) == 0 {
...
func (hc *HeaderChain) GetTd(hash common.Hash, number uint64) *big.Int {
	return hc.chainTracker.GetTd(hash, number)
...
func (hc *HeaderChain) GetTdByHash(hash common.Hash) *big.Int {
	return hc.chainTracker.GetTdByHash(hash)
...
func (hc *HeaderChain) GetHeader(hash common.Hash, number uint64) *types.Header {
	return hc.chainTracker.GetHeader(hash, number)
...
func (hc *HeaderChain) GetHeaderByHash(hash common.Hash) *types.Header {
	return hc.chainTracker.GetHeaderByHash(hash)
	//number := hc.GetBlockNumber(hash)
	//if number == nil {
	//	return nil
	//}
...
func (hc *HeaderChain) HasHeader(hash common.Hash, number uint64) bool {
	return hc.chainTracker.GetHeader(hash, number) != nil || rawdb.HasHeader(hc.chainDb, hash, number)
...
func (hc *HeaderChain) GetHeaderByNumber(number uint64) *types.Header {
	return hc.chainTracker.GetHeaderByNumber(number, hc.chainDb)
...
func (hc *HeaderChain) CurrentHeader() *types.Header {
	return hc.chainTracker.CurrentHeader()
...
func (hc *HeaderChain) SetCurrentHeader(head *types.Header) {
	if head.Number.Uint64() == 0 {
		if head.Hash() == hc.genesisHeader.Hash() {
			// no op for now
			return
		}
	}
	panic(fmt.Sprintf("SetCurrentHeader not implemented %v", head.Number))
