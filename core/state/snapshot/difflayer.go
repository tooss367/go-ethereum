...
var (
	...
	// the bloom hashes that the hasher functions looks at, to determine the
	// hash for an account/slot. This is randomized at init(), so that the
	// global population of nodes do not all display the exact same behaviour
	// with regards to bloom content
	bloomHasherOffset = 0
)

func init() {
	// init bloomHasherOffset in the range [0:24]
