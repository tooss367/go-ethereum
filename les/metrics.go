...
type meteredMsgReadWriter struct {
	types.MsgReadWriter     // Wrapped message stream to meter
	version           int // Protocol version to select correct meters
}

// newMeteredMsgWriter wraps a p2p MsgReadWriter with metering support. If the
// metrics system is disabled, this function returns the original object.
func newMeteredMsgWriter(rw types.MsgReadWriter) types.MsgReadWriter {
	if !metrics.Enabled {
		return rw
	}
...
func (rw *meteredMsgReadWriter) ReadMsg() (types.Msg, error) {
...
func (rw *meteredMsgReadWriter) WriteMsg(msg types.Msg) error {
	// Account for the data traffic
