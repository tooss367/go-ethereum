...
func (t *testPeer) RequestAccountRange(requestId uint64, root common.Hash, origin common.Hash, cap uint64) error {
	t.Log().Info("<- AccRangeReq", "req_id", requestId, "root", root,
		"origin", origin, "max", cap)
...
func (t *testPeer) RequestStorageRanges(requestId uint64, root common.Hash, accounts []common.Hash, origin []byte, max uint64) error {
	t.test.Logf("%v <- StorRangeReq{id:%d, root: %x, account[0]: %x, origin: %x, max: %d}",
		t.id, requestId, root, accounts[0], origin, max)
	// Pass the response
	go func() {
