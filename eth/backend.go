...
func New(stack *node.Node, config *Config) (*Ethereum, error) {
	...
	eth.ethDialCandidates, err = setupDiscovery(&stack.Config().P2P, eth.config.EthDiscoveryURLs)
	if err != nil {
		return nil, err
	}
	eth.snapDialCandidates, err = setupDiscovery(&stack.Config().P2P, eth.config.SnapDiscoveryURLs)
	if err != nil {
		return nil, err
	}
